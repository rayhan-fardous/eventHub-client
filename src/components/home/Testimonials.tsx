      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 sm:gap-6 lg:gap-8">
          {testimonials.map((t, index) => (
            <div
              key={index}
              className="glass-panel glass-panel-hover p-6 sm:p-8 rounded-[2.5rem] flex flex-col justify-between text-left transition-all duration-300 relative"
            >
              <Quote className="absolute top-5 right-6 size-8 sm:size-10 text-brand-cyan/10 pointer-events-none" />
              <div>
                {/* Rating stars */}
                <div className="flex gap-1 mb-3 sm:mb-4">
                  {Array.from({ length: t.rating }).map((_, i) => (
                    <Star key={i} className="size-3.5 sm:size-4 fill-yellow-400 text-yellow-400" />
                  ))}
                </div>
              ...
              {/* Author Metadata */}
              <div className="flex items-center gap-3 sm:gap-4 mt-6 sm:mt-8 pt-5 sm:pt-6 border-t border-brand-border/50 shrink-0">
                <img
                  src={t.avatar}
                  alt={t.author}
                  className="size-10 sm:size-11 rounded-full border border-brand-border object-cover bg-zinc-800 shrink-0"
                />
                <div>
