        ...
        <div className="space-y-4 text-left">
          {faqs.map((faq, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-brand-panel/40 border border-brand-border/80 rounded-2xl overflow-hidden transition-all duration-300 hover:border-brand-cyan/30"
              >
                <button
                  onClick={() => toggleFAQ(index)}
                  className="w-full px-6 py-5 text-left flex justify-between items-center gap-4 text-brand-text-primary font-bold text-base sm:text-lg cursor-pointer"
                >
                  <span>{faq.question}</span>
                  {isOpen ? (
                    <ChevronUp className="size-5 text-brand-cyan shrink-0" />
                  ) : (
                    <ChevronDown className="size-5 text-brand-text-muted shrink-0" />
                  )}
                </button>
                
                {isOpen && (
                  <div className="px-6 pb-6 pt-1 text-sm sm:text-base text-brand-text-secondary border-t border-brand-border/40 animate-in fade-in slide-in-from-top-2 duration-300 leading-relaxed">
                    {faq.answer}
                  </div>
                )}
              </div>
